...
test.describe('Redirect chain', () => {
    ...
    test('Should display correct pages after redirecting', async ({ redirectChainPage }, testInfo) => {
        ...
        try {
            ...
            await new TakeCapture().TakeCaptureAndAttach(
                `screenshots/${testInfo.title}-ERROR.png`,
                'Test ended - ERROR',
                redirectChainPage.page,
                testInfo
            )
    ...
    test.afterEach(async ({ redirectChainPage }, testInfo) => {
        ...
        if(testInfo.errors == null){
            await new TakeCapture().TakeCaptureAndAttach(
                `screenshots/${testInfo.title}-OK.png`,
                'Test ended - OK',
                redirectChainPage.page,
                testInfo
            )
